...
#para imprimir los numeros del 1 al 100 debemos ubicar solo este comando y ejecutarlo
1:100
x<-c(1:100)
print (x)
...
by_day_departures <- group_by(flights, year, month, day)
summarise (by_day_departures, delay = mean(dep_delay, na.rm = TRUE))%>%# aqui obtengo los retrasos promedio en las salidas por dia
arrange(desc(delay))

by_day_arrive <- group_by(flights, year, month, day)%>%
summarise (by_day_arrive, delay = mean(arr_delay, na.rm = TRUE))%>%# aqui obtengo los retrasos promedio en las llegadas por dia
arrange(desc(delay))

by_dest <- group_by(flights, dest)%>%## RETRASOS POR DESTINOS
delay <- summarise (by_dest,count = n(),dist = mean(distance, na.rm = TRUE),delay = mean(arr_delay, na.rm = TRUE))

delay <- filter(delay, count > 20, dest != "HNL") # Omite ONULULU INTL por la distancia, es el aeropuerto mas lejano

not_cancelled <- flights %>%
  filter(!is.na(dep_delay), !is.na(arr_delay))#se asume que los na en llegada y salida, son vuelos cancelados por que es prematuro decir que tuvieron accidentes.

not_cancelled %>%
  group_by(year, month, day) %>%  #se organizan los vuelos no cancelados para determinar la media de retrasos en salidas
  summarise(mean = mean(dep_delay)) # se podria hacer los mismo para las llegadas.

not_cancelled %>%
  group_by(year, month, day) %>%
  summarise(n_early = sum(dep_time + 15))
